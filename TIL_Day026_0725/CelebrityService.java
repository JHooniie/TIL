...
public class CelebrityService {
	...
	// 네이버 얼굴인식 API 예제
	public ArrayList<CelebrityVO> CelebrityFace() {
        
		ArrayList<CelebrityVO> celList = new ArrayList<CelebrityVO>();
		
        String clientId = "r2itlffg01";//애플리케이션 클라이언트 아이디값";
        String clientSecret = "vP3lHXBRCtkLKf9F2pf0SIz0Ca3wYLATBc9g6O5L";//애플리케이션 클라이언트 시크릿값";

        try {
            String paramName = "image"; // 파라미터명은 image로 지정
            // 이미지 직접 삽입
            String imgFile = "C:/images/song.jpg";
	...
	private ArrayList<CelebrityVO> jsonToVOList(String jsonResultStr){
	ArrayList<CelebrityVO> celList = new ArrayList<CelebrityVO>();
	
	//json 형태 문자열에서 오브젝트 추출하여 jsonArray에 저장
	...
	if(celArray != null) {
		for (int i=0; i < celArray.length(); i++) {
			...
			tempObj = tempObj.getJSONObject("celebrity");
